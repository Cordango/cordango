...
/// the model which icons exist), the compiler (validates icons + stamps keyword-based
...
///
/// <para><b>Curated is what the model is taught; the font is what is real.</b> The compiler used to
/// keep an icon only when it was curated, which also replaced icons a person chose on purpose: 133
/// of the 213 app, page and entity icons in the gallery templates never showed, and an app about
/// resource planning got an airplane because its description mentions leave. An icon is now kept
/// when <see cref="Font"/> has it, and the keyword guess only fills a missing or unknown one.</para>
/// </summary>
public static class IconCatalog
{
    /// <summary>Every name the renderer's font defines, from <c>schemas/mdi-icons.txt</c>
    /// (GENERATED by <c>catalog/export_icons.py</c> from @mdi/font; a web test fails when the two
    /// drift).</summary>
    public static readonly IReadOnlySet<string> Font = LoadFont();

    private static HashSet<string> LoadFont()
    {
        using var stream = typeof(IconCatalog).Assembly.GetManifestResourceStream("mdi-icons.txt")
            ?? throw new InvalidOperationException("mdi-icons.txt is not embedded in the compiler");
        using var reader = new StreamReader(stream);
        var names = new HashSet<string>(StringComparer.Ordinal);
        while (reader.ReadLine() is { } line)
            if (line.Length > 0 && line[0] != '#') names.Add(line.Trim());
        return names;
    }
    ...
    public static readonly IReadOnlySet<string> All = new HashSet<string>(StringComparer.Ordinal)
    {
        ...
        "timeline-clock", "sitemap", "source-branch", "swap-horizontal", "check-decagram",
    ...
    private static readonly (string Keyword, string Icon)[] Keywords =
    {
        ...
        ("approval", "check-decagram"), ("workflow", "source-branch"), ("process", "source-branch"),
    ...
    /// <summary>
    /// Resolve an icon: keep <paramref name="requested"/> when the font has it (with or without an
    /// accidental <c>mdi-</c> prefix), otherwise fall back to the first keyword hit across the given
    /// hints (key, label, …), otherwise <paramref name="fallback"/>.
    /// </summary>
    public static string Resolve(string? requested, string fallback, params string?[] hints)
    {
        ...
        if (name is { Length: > 0 })
        {
            ...
            if (All.Contains(name) || Font.Contains(name)) return name;
        }
