    ...
    private static JsonObject Field() => new()
    {
        ...
        ["properties"] = new JsonObject
        {
            ...
            ["expr"] = Str(
                ...
                + "only the branch taken runs, so `if(months == 0, 0, costs / months)` is safe. For a table, "
                + "never nest ifs: `switch(state, 'sachsen', 8450, 'bayern', 8200, 9180)` keys on one "
                + "field; a key that is not one of that field's codes, or a repeated key, is refused. "
                + "`case(test, value, test, value, default)` does thresholds, first match wins. Compare "
                + "a select or text field to a quoted code with == or != only; text is never joined, "
